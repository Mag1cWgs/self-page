...
## 比较Array和ArrayList
- 大小：数组大小固定，一旦创建就不能改变大小。而 `ArrayList` 的大小是动态的，可以根据需要增加或减少。
- 类型：数组只能存储一种类型的元素，而 `ArrayList` 可以存储不同类型的元素。
- 性能：由于 `ArrayList` 的动态特性，相对于数组，它在插入和删除元素时可能性能稍差。同时，由于 `ArrayList` 存储的是 `Object` 类型，因此在访问元素时需要进行类型转换，这也会带来额外的开销。
- 线程安全：`ArrayList` 不是线程安全的，如果需要线程安全的集合，应该使用 `System.Collections.ArrayList.Synchronized` 方法返回一个线程安全的包装器，或者直接使用 `System.Collections.Concurrent` 命名空间下的集合类。
- 泛型：随着 C# 2.0 的引入，`List<T>`（泛型列表）成为了一个更好的替代 `ArrayList` 的选择，因为它在编译时进行类型检查，避免了类型转换的开销，并且提供了更丰富的功能和更好的性能。
